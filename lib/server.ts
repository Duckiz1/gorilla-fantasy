...
export function originOK(r:Request){return r.headers.get('origin')===new URL(config('SITE_URL')||r.url).origin;}
export async function siteOwner(r:Request){const u=await user(r);const allowed=config('OWNER_DISCORD_ID').trim();return u&&allowed&&u.id===allowed?u:null;}
