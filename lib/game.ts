...
export type Tournament = { id: string; name: string; example: boolean; phase: 'seeding'|'bracket'|'complete'; seedLock: string; bracketLock: string; fantasyLock: string; teams: Team[]; players: Player[]; qualified: string[]; results: Record<string,string>; matches: {id:string;a:string;b:string;lock:string;winner?:string}[] };
export type Picks = { fantasy: string[]; seeding: string[]; bracket: Record<string,string>; matches: Record<string,string> };
export const emptyPicks = (): Picks => ({fantasy:[],seeding:[],bracket:{},matches:{}});
