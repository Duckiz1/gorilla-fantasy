...
export default function ManageRecords({matches,teams,act}:{matches:Match[];teams:Team[];act:(path:string,body:object)=>Promise<boolean>}){
 ...
 <h3>Leaderboard users</h3><p>Removing a user resets their points to 100 and clears their previous bets. They stay hidden until they place their next successful bet. Their account and team membership remain.</p>
 <button className="danger-button" disabled={busy||!players.length} onClick={()=>void run({action:'clearLeaderboard'},'Reset ALL current users to 100 points and clear their previous bets? Each user returns to the leaderboard after their next successful bet.')}>Clear leaderboard</button>
 {players.slice((page-1)*10,page*10).map(p=><div className="record-row" key={p.id}><div><strong>#{p.rank} {p.name}</strong><small>{p.points} points</small></div><button className="danger-button" disabled={busy} onClick={()=>void run({action:'removeUser',id:p.id},'Remove '+p.name+' from the leaderboard and reset them to 100 points? Their previous bets will be cleared. They return after their next successful bet.')}>Remove user</button></div>)}
