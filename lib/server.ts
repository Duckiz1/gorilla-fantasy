...
export function cookieHeader(r:Request,name:string,value:string,age:number){return `${name}=${encodeURIComponent(value)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${age}${new URL(r.url).protocol==='https:'?'; Secure':''}`;}
export function originOK(r:Request){return r.headers.get('origin')===new URL(r.url).origin;}
export async function siteOwner(r:Request){const u=await user(r);const allowed=(config('OWNER_DISCORD_ID')||config('ADMIN_DISCORD_IDS').split(',')[0]||'').trim();return u&&allowed&&u.id===allowed?u:null;}
